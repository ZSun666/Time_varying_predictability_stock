function [select_index] = EMVS_2(x,y,sigma,select_init)
    [T,K] = size(x);
    theta_n = 0.05;
    gamma_n = zeros(K,1);
    ...
    while count_k_0 <3 && count_iter < 100
        ...
        log_theta_ratio = log(a_0+sum(gamma_n))-1/(2*(a_0+sum(gamma_n)))... % E(log(theta/1-theta))
                         +  log(b_0+K+sum(gamma_n))-1/(2*(b_0+K+sum(gamma_n)));
        
        r_temp_part = (1+exp(-log_theta_ratio));             
        
        for k = 1:K
            
            E_beta_2(k,1) = m_n(k)^2+ sigma*inv_V_n(k,k);
            lik_ratio = mvnpdf(sqrt(E_beta_2(k,1)),0,v_0)/mvnpdf(sqrt(E_beta_2(k,1)),0,v_1);
            r_temp = 1/(r_temp_part*lik_ratio);
            if  r_temp >0.5
                gamma_n(k) = 1;
            else
                gamma_n(k) = 0;
