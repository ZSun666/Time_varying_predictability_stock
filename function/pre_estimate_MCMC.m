function [beta_fix,theta_fix,f_tilde,sigma_t,W_draw,W_SSE_2,tau_save,xi_save,kappa_save,lambda_save,a_tau_save,a_xi_save]...
            = pre_estimate_MCMC(Y_train,X_train,N,M,K,K_state,T_pre)
MK = M*K;
...
a_xi = 0.1*ones(K_state,1);
kappa_init = 1*ones(K_state,1);
d_1 = 0.001;
d_2 = 0.001;
...
kappa_save = zeros(K_state,N-nburn);
lambda_save = zeros(N-nburn,1);
a_tau_save = zeros(N-nburn,1);
a_xi_save = zeros(K_state,N-nburn);
    ...
    %% draw hyperparameters
         for k = 1:K_state
            ...
            for j = 1:(MK)    
                ...
                * (sqrt(a_xi_prop*kappa_draw(k))^(a_xi_prop+0.5))/(sqrt(a_xi_old*kappa_draw(k))^(a_xi_old+0.5))...
                * abs(theta_draw(jj))^(a_xi_prop-a_xi_old) * 2^(a_xi_old-a_xi_prop)...
                * gamma(a_xi_old)/gamma(a_xi_prop)...
                * besselk(a_xi_prop-0.5,sqrt(a_xi_prop*kappa_draw(k))*abs(theta_draw(jj)))...
                / besselk(a_xi_old-0.5,sqrt(a_xi_old*kappa_draw(k))*abs(theta_draw(jj)));
        ...
        for j = 1:MK
            ...
            for j = 1:MK
                jj = (k-1)*MK+j;
                xi_draw(jj) = gigrnd(a_xi(k) - 0.5,a_xi(k)*kappa_draw(k),theta_draw(jj)^2,1);
                if xi_draw(jj) ==0 || isnan(xi_draw(jj))  || isinf(xi_draw(jj))
                    xi_draw(jj) = 0.0001; %introduce some drift
                end
            end
            end
        % sample lambda&kappa
        lambda_draw = gamrnd(e_1+a_tau*MK,1/(e_2+mean(tau_draw,1)*0.5*a_tau*MK));

        for k = 1:K_state
            xi_index = [(k-1)*MK+1:k*MK];
            kappa_draw(k) = gamrnd(d_1+a_xi(k)*(MK),1/(d_2+mean(xi_draw(xi_index),1)*0.5*a_xi(k)*(MK)));
        end
        ...
        [sigma_draw,~,sigt,Wdraw]  = mcmc_draw_sigma(statedraw,Wdraw,yss,Zs,m_s,u2_s,1,Ttrain,sigma_prmean,sigma_prvar,W_prmean,W_prvar);
        W_SSE_2_draw = sigma_draw*sigma_draw';
