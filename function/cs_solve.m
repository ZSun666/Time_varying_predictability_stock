...
if sigma>0
    ...
    [xfullhat,fval,exitflag,output,lambda] = linprog(c,F,b,[],[],[],[],options);
else
    c = [zeros(m,1);
        ones(m,1)];
    ...
    b_eq_exact = y;
    [xfullhat,fval,exitflag,output,lambda] = linprog(c,F_exact,b_exact,F_eq_exact,b_eq_exact,'Display','off');
end
