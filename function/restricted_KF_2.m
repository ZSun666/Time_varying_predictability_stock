...
%     
    ...
    A = -eye(MK)*(Y-X * m_predict); b = (m_predict);
    Aeq = ones(1,MK)*(Y-X * m_predict); beq = 1-sum(m_predict); 
    ob_function = @(K)(trace((eye(MK)-K*X)*P_predict*(eye(MK)-K*X)'+K*R*K'));
    opts = optimoptions('fmincon','Display','off','Algorithm','active-set');
    K_restrict = fmincon(ob_function,KG,A,b,Aeq,beq,[],[],[],opts);
    %m_restriced = m_update;
    m_restriced = m_predict + K_restrict*(Y-X * m_predict);
    %m_restriced = m_restriced/(sum(m_restriced));
    p_restricted =  (eye(MK) - K_restrict*X)* P_predict; 
%     W = inv(P_last);
