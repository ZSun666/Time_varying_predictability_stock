...
if isinf(b) 
    % special case which is gamma distribution
        if lambda > 0
            for i = 1:sampleSize
                X(i) = gamrnd(lambda, 2/a);
            end
        else
            for i = 1:sampleSize
                X(i) = 1/gamrnd(-lambda, 2/a);
            end
        end


elseif isinf(a) 
    % special case which is inverse gamma distribution
        if lambda > 0 
            for i=1:sampleSize
                X(i) = 1/gamrnd(lambda, 2/b); 
            end

        else 
            for i=1:sampleSize
                X(i)  = gamrnd(-lambda, 2/b); 
            end
