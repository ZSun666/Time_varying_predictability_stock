%% For fix-window 
%% Besides the last draw of TVP, also save init of TVP as init in mutation step.

function [beta_fix,theta_fix,f_tilde,f_tilde_init,sigma_t,sigma_t_init,W_draw,W_SSE_2,tau_save,xi_save,kappa_save,lambda_save,a_tau_save,a_xi_save]...
            = pre_estimate_MCMC_3(Y_train,X_train,N,M,K,K_state,T_pre)
MK = M*K;
...
%             end
        ...
        % sample lambda&kappa
        lambda_draw = gamrnd(e_1+a_tau*MK,1/(e_2+mean(tau_draw,1)*0.5*a_tau*MK));
        kappa_draw = gamrnd(d_1+a_xi*(MK*K_state),1/(d_2+mean(xi_draw,1)*0.5*a_xi*(MK*K_state)));
        ...
        if n > nburn
            ...
            f_tilde_init(:,n-nburn) = f_tilde_draw(1,:);
            %X_f_tilde(:,n-nburn,:) = X_f_train';
            sigma_t(:,n-nburn,:) = sigma_draw;
            sigma_t_init(:,n-nburn) = sigma_draw(1);
